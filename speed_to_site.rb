...
class SpeedTest < Thor
  ...
  def start(args)
    ...
    results = "Over the #{runs} tests, the average time was #{avg_a.to_i}ms, the perc95 was #{perc95_a.to_i}ms and the perc99 was #{perc99_a.to_i}ms."
    
   File.open("out.txt",'a') {|f| 
     f.puts results
   }
    puts results + " Detailed output is in file out.txt"
  end
end
